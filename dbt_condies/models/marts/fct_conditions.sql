...
pivoted as (
    select
        ...
        fetch_date,
        day,
        max(case when window = 'morning'   then min_temp_f        end) as morning_min_temp_f,
        max(case when window = 'morning'   then avg_temp_f        end) as morning_avg_temp_f,
        max(case when window = 'morning'   then max_temp_f        end) as morning_max_temp_f,
        max(case when window = 'morning'   then avg_dewpoint_f    end) as morning_avg_dewpoint_f,
        max(case when window = 'morning'   then total_precip_in   end) as morning_precip_in,
        max(case when window = 'morning'   then avg_windspeed_mph end) as morning_windspeed_mph,
        max(case when window = 'afternoon' then min_temp_f        end) as afternoon_min_temp_f,
        max(case when window = 'afternoon' then avg_temp_f        end) as afternoon_avg_temp_f,
        max(case when window = 'afternoon' then max_temp_f        end) as afternoon_max_temp_f,
        max(case when window = 'afternoon' then avg_dewpoint_f    end) as afternoon_avg_dewpoint_f,
        max(case when window = 'afternoon' then total_precip_in   end) as afternoon_precip_in,
        max(case when window = 'afternoon' then avg_windspeed_mph end) as afternoon_windspeed_mph,
        max(case when window = 'evening'   then min_temp_f        end) as evening_min_temp_f,
        max(case when window = 'evening'   then avg_temp_f        end) as evening_avg_temp_f,
        max(case when window = 'evening'   then max_temp_f        end) as evening_max_temp_f,
        max(case when window = 'evening'   then avg_dewpoint_f    end) as evening_avg_dewpoint_f,
        max(case when window = 'evening'   then total_precip_in   end) as evening_precip_in,
        max(case when window = 'evening'   then avg_windspeed_mph end) as evening_windspeed_mph
    from windows
